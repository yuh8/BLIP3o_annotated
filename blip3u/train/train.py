...
from blip3u.constants import IGNORE_INDEX, DEFAULT_IMAGE_TOKEN, IMAGE_TOKEN_IDX
from torch.utils.data import Dataset
from blip3u.train.blip3u_trainer import blip3uTrainer

from blip3u import conversation as conversation_lib
from blip3u.model import *
from blip3u.mm_utils import tokenizer_image_token
...
class LazySupervisedMixDataset(Dataset):
    ...
    def __init__(
        ...
        print(f"finish loading getty image {len(train_dataset)}")
...
def train(attn_implementation=None):
    ...
    if model_args.vision_tower is not None:
        model = blip3uLlamaForCausalLM.from_pretrained(
        ...
        if "Qwen" in model_args.model_name_or_path or "qwen" in model_args.model_name_or_path :
            model = blip3uQwenForCausalLM.from_pretrained(
                model_args.model_name_or_path,
                cache_dir=training_args.cache_dir,
                attn_implementation=attn_implementation,
                torch_dtype=(torch.bfloat16 if training_args.bf16 else None),
            ...
            model = transformers.LlamaForCausalLM.from_pretrained(
                ...
                attn_implementation=attn_implementation,
                torch_dtype=(torch.bfloat16 if training_args.bf16 else None),
                use_auth_token="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",
                **bnb_model_from_pretrained_args,
            )
    ...
    class UploadCheckpointCallback(TrainerCallback):
        def on_save(self, args, state, control, **kwargs):
            """
            This function is triggered on each checkpoint save during training.
            """
            if not state.is_world_process_zero:
                return

            if not hasattr(args, "uploaded_checkpoint"):
                args.uploaded_checkpoint = [0]

            if state.global_step // 10000 in args.uploaded_checkpoint:
                return
            args.uploaded_checkpoint.append(state.global_step // 10000)
            api = HfApi()
            api.create_repo(
                repo_id=f"umd-vt-nyu/{args.output_dir.split('/')[-1]}",
                repo_type="model",
                exist_ok=True,
            )
            ...
            upload_folder(
                ...
                repo_type="model",
                ignore_patterns=["checkpoint-*/global_step*"],
            )

    trainer = blip3uTrainer(
        model=model,
        tokenizer=tokenizer,
        args=training_args,
        **data_module,
