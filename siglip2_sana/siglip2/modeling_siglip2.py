...
class Siglip2PreTrainedModel(PreTrainedModel):
    ...
    def _init_weights(self, module):
        ...
        if isinstance(module, Siglip2VisionEmbeddings):
            width = self.config.vision_config.hidden_size if isinstance(self.config, Siglip2Config) else self.config.hidden_size
...
class Siglip2Model(Siglip2PreTrainedModel):
    ...
    def __init__(self, config: Siglip2Config):
        ...
        if not isinstance(config.text_config, Siglip2TextConfig):
            raise TypeError("config.text_config is expected to be of type Siglip2TextConfig but is of type" f" {type(config.text_config)}.")

        if not isinstance(config.vision_config, Siglip2VisionConfig):
            raise TypeError("config.vision_config is expected to be of type Siglip2VisionConfig but is of type" f" {type(config.vision_config)}.")
